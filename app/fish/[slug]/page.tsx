...
const primaryMethod:Record<string,string>={tachiuo:'tachiuo-tenya',aji:'sabiki',madai:'tai-rubber',saba:'sabiki',buri:'nomase',kisu:'choinage',kasago:'ana',kawahagi:'kawahagi',hirame:'nomase',aoriika:'eging',sawara:'blade-jig',hamachi:'shore-jigging',mebaru:'mebaring',madako:'tako-egi',chinu:'fukase',unagi:'bukkomi'};
const methodHref:Record<string,string>={
 'テンヤ':'/methods#tachiuo-tenya','船テンヤ':'/methods#tachiuo-tenya','サビキ':'/methods#sabiki','サビキ釣り':'/methods#sabiki','タイラバ':'/methods#tai-rubber','ノマセ':'/methods#nomase','泳がせ':'/methods#nomase',
 'ジギング':'/methods#jigging','ウキ釣り':'/methods#uki','引き釣り':'/methods#tachiuo-tenya-shore','アジング':'/methods#ajing','船釣り':'/methods#douzuki','ひとつテンヤ':'/methods#hitotsu-tenya','コマセ':'/methods#fukase','コマセ釣り':'/methods#fukase','ちょい投げ':'/methods#choinage','船キス':'/methods#boat-kisu','胴突き':'/methods#douzuki','胴突き釣り':'/methods#douzuki','穴釣り':'/methods#ana','ルアー':'/methods#surf-lure','フカセ':'/methods#fukase','フカセ釣り':'/methods#fukase','落とし込み':'/methods#otoshikomi','ダンゴ':'/methods#fukase','メバリング':'/methods#mebaring','エギング':'/methods#eging','ティップラン':'/methods#tiprun','タコエギ':'/methods#tako-egi','キャスティング':'/methods#casting','ブレードジグ':'/methods#blade-jig','ブレードジギング':'/methods#blade-jig','ぶっこみ釣り':'/methods#bukkomi','ショアジギング':'/methods#shore-jigging'
};
...
function TachiuoPage(){
 ...
 </section><section className={t.cta}><div><h2>この魚を釣ってみよう！</h2><p>釣り場・釣り方・釣具をUOLINKでそのまま確認できます。</p></div><Link href="/spots">📍 おすすめの釣り場</Link><Link href="/gear#tachiuo">🧰 必要な釣具</Link><Link href="/methods/tachiuo-tenya">🎣 テンヤの詳細</Link><Link className={t.game} href="/game">🎮 ゲームで練習</Link></section><section className={t.lightSection}><LaunchFishExtras slug="tachiuo" name="タチウオ"/></section></div>
}
...
function AjiPage(){
 ...
 return <div className={t.page}><section className={t.hero}>
  ...
  <div className={t.stage}><div className={t.stageTabs}><span className={t.active}>▧ 2D画像</span><span>3D準備中</span></div><div className={t.viewerWrap}><FishVisual slug="aji" name="マアジ"/></div></div><FishActions slug="aji" name="マアジ" className={t.tools}/></div>
  <div className={t.cards}><article className={t.card}><h3>▣ 特徴</h3><p>{f.desc} {d.body}</p><p><b>見分けるポイント：</b>尾の付け根付近に並ぶ硬いウロコ「ゼイゴ」が代表的です。</p></article><article className={t.card}><h3>🐟 生態</h3><div className={t.list}><div><span>生息域</span><b>{d.habitat}</b></div><div><span>水深</span><b>{f.depth}</b></div><div><span>エサ</span><b>{d.diet}</b></div><div><span>行動</span><b>{d.behavior}</b></div><div><span>寿命</span><b>{d.lifespan}</b></div></div></article><article className={t.card}><h3>🎣 釣り方</h3><div className={t.methods}><Link href="/methods/sabiki">サビキ釣り <span>›</span></Link><Link href="/methods#ajing">アジング <span>›</span></Link><Link href="/methods#douzuki">船の胴突き <span>›</span></Link></div></article><article className={t.card}><h3>◷ 釣れる時期</h3><div className={t.months}>{months.map(m=><span key={m} className={f.months.includes(m)?t.on:''}>{m}月</span>)}</div><p>{d.seasonNote}</p></article></div>
 ...
 </section><section className={t.cta}><div><h2>マアジを釣ってみよう！</h2><p>初めてならサビキから。群れのいる棚を見つければ数釣りも狙えます。</p></div><Link href="/spots">📍 おすすめの釣り場</Link><Link href="/gear#aji">🧰 必要な釣具</Link><Link href="/methods/sabiki">🎣 サビキの詳細</Link><Link className={t.game} href="/game">🎮 ゲームで練習</Link></section><section className={t.lightSection}><LaunchFishExtras slug="aji" name="マアジ"/></section></div>
}
...
function MadaiPage(){
 ...
 return <div className={t.page}><section className={t.hero}>
  ...
  <div className={t.stage}><div className={t.stageTabs}><span className={t.active}>▧ 2D画像</span><span>3D準備中</span></div><div className={t.viewerWrap}><FishVisual slug="madai" name="マダイ"/></div></div><FishActions slug="madai" name="マダイ" className={t.tools}/></div>
  <div className={t.cards}><article className={t.card}><h3>▣ 特徴</h3><p>{f.desc} {d.body}</p></article><article className={t.card}><h3>🐟 生態</h3><div className={t.list}><div><span>生息域</span><b>{d.habitat}</b></div><div><span>水深</span><b>{f.depth}</b></div><div><span>エサ</span><b>{d.diet}</b></div><div><span>行動</span><b>{d.behavior}</b></div><div><span>寿命</span><b>{d.lifespan}</b></div></div></article><article className={t.card}><h3>🎣 釣り方</h3><div className={t.methods}><Link href="/methods/tai-rubber">タイラバ <span>›</span></Link><Link href="/methods#hitotsu-tenya">ひとつテンヤ <span>›</span></Link><Link href="/methods#fukase">コマセ釣り <span>›</span></Link></div></article><article className={t.card}><h3>◷ 釣れる時期</h3><div className={t.months}>{months.map(m=><span key={m} className={f.months.includes(m)?t.on:''}>{m}月</span>)}</div></article></div>
 ...
 </section><section className={t.cta}><div><h2>マダイを釣ってみよう！</h2><p>まずはタイラバから。釣り場・釣り方・釣具をそのまま確認できます。</p></div><Link href="/spots">📍 おすすめの釣り場</Link><Link href="/gear#madai">🧰 必要な釣具</Link><Link href="/methods/tai-rubber">🎣 タイラバの詳細</Link><Link className={t.game} href="/game">🎮 ゲームで練習</Link></section><section className={t.lightSection}><LaunchFishExtras slug="madai" name="マダイ"/></section></div>
}

export default async function Page({params}:{params:Promise<{slug:string}>}){
 const {slug}=await params;const f=getFish(slug);if(!f)notFound();if(slug==='tachiuo')return <TachiuoPage/>;if(slug==='aji')return <AjiPage/>;if(slug==='madai')return <MadaiPage/>;const d=fishDetails[slug];const months=Array.from({length:12},(_,i)=>i+1);const methodTarget=primaryMethod[slug];
 return <div className={t.page}><section className={t.hero}>
  <div className={t.crumb}><Link href="/">ホーム</Link>　›　<Link href="/fish">魚図鑑</Link>　›　{f.name}</div>
  <div className={t.top}><aside className={t.summary}><h1>{f.name}</h1><div className={t.en}>{f.en}</div><div className={t.facts}>{d&&<div className={t.row}><span>分類</span><b>{d.family} {d.genus}</b></div>}<div className={t.row}><span>学名</span><b><i>{f.scientific}</i></b></div>{d&&<div className={t.row}><span>別名</span><b>{d.aliases.join('・')}</b></div>}<div className={t.row}><span>体長</span><b>{f.size}</b></div><div className={t.row}><span>旬</span><b>{f.season}</b></div><div className={t.row}><span>難易度</span><b className={t.stars}>{'★'.repeat(f.difficulty)}{'☆'.repeat(5-f.difficulty)}</b></div><div className={t.row}><span>注意</span><b>{f.danger}</b></div></div><div className={t.tags}><span>{f.beginner?'初心者にもおすすめ':'経験者向け'}</span><span>{f.areas[0]}</span><span>{f.methods[0]}</span></div></aside>
  <div className={t.stage}><div className={t.stageTabs}><span className={t.active}>▧ 魚体画像</span></div><div className={t.viewerWrap}><FishVisual slug={slug} name={f.name}/></div></div><FishActions slug={slug} name={f.name} className={t.tools}/></div>
  <div className={t.cards}><article className={t.card}><h3>▣ 特徴</h3><p>{f.desc}{d?` ${d.body}`:''}</p></article><article className={t.card}><h3>🐟 生態</h3><div className={t.list}><div><span>生息域</span><b>{d?.habitat||f.areas.join('・')}</b></div><div><span>水深</span><b>{f.depth}</b></div>{d&&<><div><span>エサ</span><b>{d.diet}</b></div><div><span>行動</span><b>{d.behavior}</b></div><div><span>寿命</span><b>{d.lifespan}</b></div></>}</div></article><article className={t.card}><h3>🎣 釣り方</h3><div className={t.methods}>{f.methods.map(m=><Link href={methodHref[m]||'/methods'} key={m}>{m} <span>›</span></Link>)}</div></article><article className={t.card}><h3>◷ 釣れる時期</h3><div className={t.months}>{months.map(m=><span key={m} className={f.months.includes(m)?t.on:''}>{m}月</span>)}</div></article></div>
 </section>
 <section className={t.cta}><div><h2>{f.name}を釣ってみよう！</h2><p>重要情報を確認したら、釣り方・釣り場・釣具へ進めます。</p></div><Link href="/spots">📍 おすすめの釣り場</Link><Link href={`/gear#${slug}`}>🧰 必要な釣具</Link><Link href={methodTarget?`/methods#${methodTarget}`:'/methods'}>🎣 釣り方を見る</Link><Link className={t.game} href="/game">🎮 ゲームで遊ぶ</Link></section>
 <section className={t.lightSection}>
  {d?<><section className="detailGrid"><article><span>BEGINNER GUIDE</span><h2>初めて狙うなら</h2><p>{d.beginnerTip}</p><h3>釣り方のポイント</h3><p>{d.fishing}</p></article><aside><span>SAFETY</span><h2>扱い方と注意点</h2><p>{d.safety}</p><h3>主なエリア</h3><p>{f.areas.join('・')}</p></aside></section><section className="detailGrid"><article><span>TABLE</span><h2>食べて楽しむ</h2><p>{d.taste}</p><div className="chips">{d.cooking.map(x=><span key={x}>{x}</span>)}</div></article><aside><span>TRIVIA</span><h2>魚の豆知識</h2><p>{d.trivia}</p>{d.aliases.length>0&&<><h3>別名</h3><p>{d.aliases.join('・')}</p></>}</aside></section></>:<section className="detailGrid"><article><span>FISHING GUIDE</span><h2>{f.name}を狙うときの基本</h2><p>{f.desc}</p><h3>代表的な釣り方</h3><p>{f.methods.join('・')}</p></article><aside><span>FIELD NOTE</span><h2>覚えておきたいこと</h2><h3>主なエリア</h3><p>{f.areas.join('・')}</p><h3>水深の目安</h3><p>{f.depth}</p><h3>注意点</h3><p>{f.danger}</p></aside></section>}
  <LaunchFishExtras slug={slug} name={f.name}/>
 </section>
 </div>
}