...
export default async function Page({params}:{params:Promise<{slug:string}>}){
 const {slug}=await params;const f=getFish(slug);if(!f)notFound();if(slug==='tachiuo')return <TachiuoPage/>;if(slug==='aji')return <AjiPage/>;if(slug==='madai')return <MadaiPage/>;const d=fishDetails[slug];const months=Array.from({length:12},(_,i)=>i+1);
 return <div className={t.page}><section className={t.hero}>
  ...
  <div className={t.cards}><article className={t.card}><h3>▣ 特徴</h3><p>{f.desc}{d?` ${d.body}`:''}</p></article><article className={t.card}><h3>🐟 生態</h3><div className={t.list}><div><span>生息域</span><b>{d?.habitat||f.areas.join('・')}</b></div><div><span>水深</span><b>{f.depth}</b></div>{d&&<><div><span>エサ</span><b>{d.diet}</b></div><div><span>行動</span><b>{d.behavior}</b></div><div><span>寿命</span><b>{d.lifespan}</b></div></>}</div></article><article className={t.card}><h3>🎣 釣り方</h3><div className={t.methods}>{f.methods.map(m=><Link href="/methods" key={m}>{m} <span>›</span></Link>)}</div></article><article className={t.card}><h3>◷ 釣れる時期</h3><div className={t.months}>{months.map(m=><span key={m} className={f.months.includes(m)?t.on:''}>{m}月</span>)}</div></article></div>
 </section>
 <section className={t.cta}><div><h2>{f.name}を釣ってみよう！</h2><p>重要情報を確認したら、釣り方・釣り場・釣具へ進めます。</p></div><Link href="/spots">📍 おすすめの釣り場</Link><Link href="/gear">🧰 必要な釣具</Link><Link href="/methods">🎣 釣り方を見る</Link><Link className={t.game} href="/game">🎮 ゲームで遊ぶ</Link></section>
