...
export async function POST(request:Request){try{sameOrigin(request);const user=await requireCatchModerator();const v=await limitedJson(request,2000) as Record<string,unknown>;if(!validDeleteToken(v.id)||!['approve','reject','delete','resolve'].includes(v.action as string))throw new HttpError(400,'操作を確認してください。');await takeQuota(request,'catch-moderate',30,user.id);await moderateCatch(v.id,v.action as 'approve'|'reject'|'delete'|'resolve');return Response.json({updated:true});}catch(e){return apiError(e)}}
