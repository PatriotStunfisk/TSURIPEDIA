...
import {catchStoreReady,publishCatch,recentCatches,recentCatchSummary,ownCatches,deleteStoredCatch,takeQuota} from '@/lib/server/catch-store';
...
export async function GET(request:Request){try{if(!catchStoreReady())return Response.json({available:false,reports:[]},{headers:{'Cache-Control':'no-store'}});const url=new URL(request.url),spot=url.searchParams.get('spot')??'',offset=Number(url.searchParams.get('offset')??0);if((spot&&!allSpots.has(spot))||!Number.isInteger(offset)||offset<0||offset>10000)throw new HttpError(400,'釣り場・表示位置が不正です。');const mine=url.searchParams.get('mine')==='1';if(!mine&&!spot)throw new HttpError(400,'釣り場を選択してください。');const reports=mine?await ownCatches((await catchUser())!.id,spot||undefined,offset):url.searchParams.get('summary')==='1'?await recentCatchSummary(spot):await recentCatches(spot,offset);return Response.json({available:true,reports},{headers:{'Cache-Control':'private, no-store'}});}catch(e){return apiError(e)}}
