...
export default async function Page({params}:{params:Promise<{slug:string}>}){
 ...
 return <div className="section pageTop">
  ...
  {slug==='tachiuo-tenya'&&<div className="adNotice">このページにはAmazonアソシエイトのアフィリエイトリンクを含みます。</div>}
  ...
  <section className="detailGrid"><article><span>OVERVIEW</span><h2>{m.name}とは？</h2><p>{m.overview}</p><h3>タックル</h3><p><b>ロッド：</b>{m.rod}</p>{slug==='tachiuo-tenya'&&<a href={TACHIUO_ROD_AMAZON} target="_blank" rel="sponsored noopener noreferrer" className="gearCta">Amazonで船タチウオテンヤロッドを見る →</a>}<p><b>リール：</b>{m.reel}</p><p><b>ライン：</b>{m.line}</p><p><b>リーダー：</b>{m.leader}</p></article><aside><span>RIG & BAIT</span><h2>仕掛けとエサ</h2><p><b>仕掛け：</b>{m.rig}</p><p><b>エサ：</b>{m.bait}</p><Link href="/gear" className="gearCta">この釣りの釣具を見る →</Link></aside></section>
