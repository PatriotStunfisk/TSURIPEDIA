...
const allMethods=[...methods,...moreMethods];
export default function Page(){return <div className="section pageTop"><style>{`@media(max-width:700px){.method-list article{grid-template-columns:38px 54px 1fr!important;gap:10px!important;padding:20px 16px!important}.method-list article>.method-cta{grid-column:2/4!important;width:100%!important;display:flex!important;justify-content:center!important;align-items:center!important;white-space:nowrap!important;margin-top:4px!important}.method-list .listEmoji{font-size:30px!important}.method-list h2{font-size:23px!important}.method-list p{line-height:1.7!important}.method-list .chips{gap:5px!important}}`}</style><div className="pageHero"><span>FISHING METHODS</span><h1>釣り方</h1><p>同じ魚でも、船と堤防ではタックルも仕掛けも別物。まず「どこから釣るか」を分けて選べるようにしています。</p></div><div className="listCards method-list">{allMethods.map((m,i)=>{const d=methodDetails[m.slug];return <article key={m.slug}><div className="number">{String(i+1).padStart(2,'0')}</div><div className="listEmoji">{m.emoji}</div><div><div className="chips"><span>{placeIcon(m.place)} {m.place}</span><span>{m.level}</span></div><h2>{m.name}</h2><p>{m.desc}</p><div className="chips"><span>狙える魚：{m.target}</span>{d&&<span>手順 {d.steps.length}ステップ</span>}{d&&<span>難易度 {'★'.repeat(d.difficulty)}</span>}</div></div>{d?<Link href={`/methods/${m.slug}`} className="searchBtn method-cta">{placeIcon(m.place)} {m.place}の釣り方を見る →</Link>:<Link href={`/fish`} className="searchBtn method-cta">この釣りで狙える魚を見る →</Link>}</article>})}</div></div>}
