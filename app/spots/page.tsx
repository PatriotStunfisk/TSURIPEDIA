...
export default async function Page({searchParams}:{searchParams:Promise<{q?:string;type?:string}>}){
  ...
  return <div className="section pageTop">
    ...
    <SpotMap mapEntries={fishingMapEntries} guideNames={Object.fromEntries(allGuides.map(g=>[g.slug,g.title]))} cookingSlugs={fishCatalog.filter(f=>getFishSpecies(f.slug)?.cooking?.recipes.length).map(f=>f.slug)} initialKind={query.type==='boat'?'boat':'all'} fishNames={Object.fromEntries(fishCatalog.map(f=>[f.slug,f.name]))} methodNames={Object.fromEntries(Object.values(methodDetails).map(m=>[m.slug,m.name]))} key={`${query.q??''}:${query.type??''}`} initialQuery={typeof query.q==='string'?query.q.slice(0,100):''}/>
  </div>;
}
