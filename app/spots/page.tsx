...
export default async function Page({searchParams}:{searchParams:Promise<{q?:string;type?:string;near?:string;shops?:string;favorites?:string;fish?:string;method?:string}>}){
  ...
  return <div className="section pageTop">
    ...
    <SpotMap initialFish={fish} initialMethod={method} fishOptions={fishOptions} guideIndex={spotGuideIndex} initialFavorites={query.favorites==='1'} shops={tackleShops} nearSpot={typeof query.near==='string'?query.near:undefined} initialShops={query.shops==='1'} mapEntries={fishingMapEntries} guideNames={Object.fromEntries(allGuides.map(g=>[g.slug,g.title]))} cookingSlugs={fishCatalog.filter(f=>getFishSpecies(f.slug)?.cooking?.recipes.length).map(f=>f.slug)} initialKind={query.type==='boat'?'boat':'all'} fishNames={Object.fromEntries(fishCatalog.map(f=>[f.slug,f.name]))} methodNames={Object.fromEntries(Object.values(methodDetails).map(m=>[m.slug,m.name]))} key={`${fish}:${method}:${query.q??''}:${query.type??''}:${query.near??''}:${query.shops??''}:${query.favorites??''}`}  initialQuery={typeof query.q==='string'?query.q.slice(0,100):''}/>
  </div>;
}
