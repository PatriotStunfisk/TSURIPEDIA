...
export const metadata=pageSharing("/cooking","魚料理・捌き方","魚ごとの代表料理・捌き方・生食の注意を確認。魚図鑑から釣った後の食卓へつながる料理ガイド。");

export default function CookingPage(){return <div className="section pageTop"><div className="pageHero"><span>UOLINK KITCHEN</span><h1>魚料理・捌き方</h1><p>釣った魚を、持ち帰ったその先まで。魚種ごとに「捌き方」と「料理」をまとめる。</p></div><div className="cookingIndexSections" style={{display:'grid',gap:28,marginTop:30}}>{cookingFish.map(f=><section key={f.slug} className="cookingFishSection" style={{padding:26,borderRadius:22,background:'#0d2d40',color:'#fff'}}><span style={{fontSize:11,fontWeight:900,letterSpacing:1.5,color:'#69c9f5'}}>FISH KITCHEN</span><h2 style={{fontSize:30,margin:'5px 0 18px'}}>{f.name}</h2><div className="cookingFishLayout"><Link href={`/cooking/${f.slug}`} className="cookingPrepCard"><b style={{fontSize:20}}>🔪 {getFishProfile(f.slug)?.hazard?"調理前の確認":"捌き方"}</b><p style={{lineHeight:1.7,opacity:.82}}>{getFishProfile(f.slug)?.hazard?"調理前の注意と、使える食材の条件を確認する →":"下処理から三枚おろしまで確認する →"}</p></Link><div className="cookingRecipeGrid">{f.recipes.map(r=><Link key={r.slug} href={`/cooking/${f.slug}/${r.slug}`} className="cookingRecipeCard">{r.image&&<img src={r.image} alt={`${f.name}の${r.name}`}/>}<div><b>{r.name}</b><small>レシピを見る →</small></div></Link>)}</div></div></section>)}</div><style>{`
