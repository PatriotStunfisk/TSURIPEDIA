...
import {gearKindLabels,gearCatalog,getGearProduct,gearVerifiedAt} from '@/lib/gear-catalog';
...
export default async function Page({params}:{params:Promise<{slug:string}>}){
 const p=getGearProduct((await params).slug);if(!p)notFound();const methods=p.methods.map(getMethod).filter(m=>!!m),fish=p.fish.map(getFishProfile).filter(f=>!!f);const guides=allGuides.filter(g=>g.methodTags.some(m=>p.methods.includes(m))&&/ライン|リール|ロッド|エギ|スナップ|仕掛け/.test(g.title)).slice(0,4);const related=p.relatedGear?p.relatedGear.map(getGearProduct).filter(g=>!!g):gearCatalog.filter(g=>((p.kind==='rod'&&g.kind==='reel')||(p.kind==='reel'&&g.kind==='rod')||(['lure','egi'].includes(p.kind)&&['rod','reel'].includes(g.kind)))&&g.methods.some(m=>p.methods.includes(m))).slice(0,3);
 return <main className="section pageTop"><Link href="/gear">← 釣具図鑑</Link><div className={s.detail}><div><GearProductVisual product={p}/>{p.image&&<small>{p.image.credit}</small>}<p className={s.note}><a href={p.source} target="_blank" rel="noopener noreferrer">メーカー公式の写真・製品情報 ↗</a></p></div><div><small className={s.eyebrow}>{p.brand} · {gearKindLabels[p.kind]}</small><h1>{p.name}</h1><p className={s.intro}>{p.summary}</p><dl className={s.specs}>{Object.entries(p.specs).map(([k,v])=><div key={k}><dt>{k}</dt><dd>{v}</dd></div>)}</dl><small className={s.note}>仕様確認：{gearVerifiedAt} / メーカー公表値</small></div></div>
 ...
 <section className={s.section}><h2>ライン・小物の組み合わせ</h2><p className={s.note}>以下はUOLINKの用途別の組み合わせ例です。</p><h3>ラインとリーダー</h3><p>{p.line}</p><h3>一緒に用意するもの</h3><p>{p.accessory}</p>{related.length>0&&<><h3>関連するロッド・リール</h3><div className={s.links}>{related.map(g=><Link href={`/gear/${g.slug}`} key={g.slug}>{g.name} →</Link>)}</div></>}<p>{p.check}</p></section>
