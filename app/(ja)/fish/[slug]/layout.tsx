...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}):Promise<Metadata>{
  ...
  return {
    ...
    openGraph:{
      url:base+url,
      title:f.hazard?`${f.name}の見分け方・危険部位｜UOLINK（ウオリンク）魚図鑑`:`${f.name}の釣り方・旬・特徴｜UOLINK（ウオリンク）魚図鑑`,
      description:f.hazard?f.hazard.headline:`${f.name}の特徴、旬、釣れる時期、釣り方、仕掛け、必要な釣具をUOLINK（ウオリンク）で紹介。${f.desc}`,
      type:'article',
...
export default async function FishLayout({children,params}:{children:React.ReactNode;params:Promise<{slug:string}>}){
  ...
  const page={
    ...
    '@id':`${url}#webpage`,
    url,
    name:f.hazard?`${f.name}の見分け方・危険部位｜UOLINK（ウオリンク）魚図鑑`:`${f.name}の釣り方・旬・特徴｜UOLINK（ウオリンク）魚図鑑`,
    description:f.hazard?f.hazard.headline:`${f.name}の特徴、旬、釣れる時期、釣り方、仕掛け、必要な釣具を紹介。${f.desc}`,
    inLanguage:'ja-JP',
