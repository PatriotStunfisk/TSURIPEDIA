...
export default function GuidePage(){
 ...
 return <div id="top" className="section pageTop">
  ...
  {cats.map(cat=><section id={catId(cat)} key={cat} style={{margin:'36px 0',scrollMarginTop:90}}><div style={{marginBottom:14}}><span style={{fontSize:12,fontWeight:900,letterSpacing:1.4,color:'#087bc4'}}>SEARCH GUIDE</span><h2 style={{margin:'4px 0 0'}}>{cat}</h2></div><div className="listCards">{allGuides.filter(a=>a.category===cat).map((a,i)=><article key={a.slug} style={{gridTemplateColumns:'42px 1fr auto'}}><div className="number">{String(i+1).padStart(2,'0')}</div><div><div className="chips"><span>{a.query}</span></div><h3 style={{fontSize:22,margin:'9px 0 8px'}}>{a.title}</h3><p>{a.summary}</p></div><Link href={`/guide/${a.slug}`} className="searchBtn">詳しく読む →</Link></article>)}</div><div style={{textAlign:'right',marginTop:12}}><a href="#top" style={{fontSize:12,fontWeight:800,color:'#087bc4'}}>ページ上部へ ↑</a></div></section>)}
 </div>
}
