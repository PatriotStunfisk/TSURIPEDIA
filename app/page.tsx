...
export default function Home(){
  ...
  return <div className={s.home}>
    ...
    <section className={s.linkHub}><div><span>EXPLORE UOLINK</span><h2>目的からすぐ探す</h2><p>ページの最後からも、GUIDE・魚・釣り方・釣り場・釣具へすぐ戻れます。</p></div><nav>{quickLinks.map(x=><Link href={x.href} key={x.href}><span>{x.icon}</span><b>{x.label}</b><em>→</em></Link>)}</nav></section>
  </div>
}
