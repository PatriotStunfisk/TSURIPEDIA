...
- Treat the established seven species as the visual baseline. The user authorizes batches of 10–20 full species per development cycle. Preserve completeness and verify each batch plus the established seven; promote existing thin profiles instead of creating duplicates.
...
- Preserve the legacy `/game` URL as a redirect to the official `/quest` experience (explicitly authorized). Public QUEST entry routes are indexable; personal collection/profile routes are noindex and excluded from sitemap. Test fish/method/gear/spot/cooking/GUIDE links and mobile gameplay, rewards, reload persistence, and collection synchronization before publishing.
