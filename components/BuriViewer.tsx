...
export default function BuriViewer(){
 ...
 useEffect(()=>{
  ...
  }catch(e){setStatus('failed');setDetail('WebGLを開始できませんでした');return}
  ...
  const fit=(obj:THREE.Object3D)=>{const box=new THREE.Box3().setFromObject(obj),size=box.getSize(new THREE.Vector3()),center=box.getCenter(new THREE.Vector3());obj.position.sub(center);const max=Math.max(size.x,size.y,size.z)||1;obj.scale.setScalar(3.55/max);camera.position.set(0,0,5.45);camera.lookAt(0,0,0)};
  ...
  loader.load('/models/fish/buri.glb?v=20260912-2',g=>{
   if(disposed)return;
   g.scene.traverse(o=>{if((o as THREE.Mesh).isMesh){const mesh=o as THREE.Mesh;mesh.frustumCulled=false}});
   holder.add(g.scene);fit(g.scene);setStatus('ready');setDetail('');
