...
export default function CatchFeed({options,prefectures,initialSpot}:{initialSpot?:string;options:CatchOptions;prefectures:string[]}){
 const session=useCatchSession();const [filters,setFilters]=useState({fish:'',method:'',prefecture:'',spot:initialSpot??'',from:'',to:'',minSize:'',maxSize:''}),[reports,setReports]=useState<PublicCatch[]>([]),[error,setError]=useState(''),[loading,setLoading]=useState(true),[more,setMore]=useState(false),[page,setPage]=useState(0),[revision,setRevision]=useState(0);
 ...
 return <section aria-label="新着釣果"><p>魚・場所・日付で絞り込めます。UOLINK投稿と公式・外部情報は区分して表示しています。</p><div className={s.feedFilters}>
 ...
 <label className={s.field}>サイズ下限（cm）<input type="number" min="0.1" max="500" step="0.1" inputMode="decimal" value={filters.minSize} onChange={e=>change('minSize',e.target.value)}/></label><label className={s.field}>サイズ上限（cm）<input type="number" min="0.1" max="500" step="0.1" inputMode="decimal" value={filters.maxSize} onChange={e=>change('maxSize',e.target.value)}/></label><label className={s.field}>釣行日（開始）<input type="date" value={filters.from} onChange={e=>change('from',e.target.value)}/></label><label className={s.field}>釣行日（終了）<input type="date" value={filters.to} onChange={e=>change('to',e.target.value)}/></label></div>
 <div className={s.actions}>{[7,30].map(days=><button key={days} className={`${s.button} ${s.secondary}`} onClick={()=>{setPage(0);setReports([]);setFilters(v=>({...v,from:new Intl.DateTimeFormat('en-CA',{timeZone:'Asia/Tokyo'}).format(new Date(Date.now()-(days-1)*86400000)),to:''}));}}>直近{days}日</button>)}<button className={s.textButton} onClick={()=>{setPage(0);setReports([]);setFilters({fish:'',method:'',prefecture:'',spot:'',from:'',to:'',minSize:'',maxSize:''});}}>条件をリセット</button></div>
 <ExternalCatchFeed query={key} options={options}/><h3>UOLINKユーザー投稿</h3>
