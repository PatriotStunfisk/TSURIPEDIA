...
import SabaViewer from './SabaViewer';

export default function SabaMedia(){
 ...
 return <div className="sabaMedia" style={{position:'absolute',inset:0,overflow:'hidden'}}>
  ...
  <div style={{position:'absolute',inset:0,padding:'54px 22px 48px',display:'flex',alignItems:'center',justifyContent:'center'}}>
   {mode==='2d'?<img src={establishedFishImages.saba} alt="マサバの図鑑画像" style={{display:'block',width:'100%',height:'100%',objectFit:'contain',filter:'drop-shadow(0 14px 18px rgba(0,20,32,.28))'}}/>:<SabaViewer/>}
  </div>
  {mode==='2d'&&<div style={{position:'absolute',left:'50%',bottom:18,transform:'translateX(-50%)',zIndex:6,color:'#c9ebf7',fontSize:10,background:'#06203099',border:'1px solid #2e566b',padding:'6px 10px',borderRadius:999,whiteSpace:'nowrap'}}>高精細2D図鑑画像</div>}
 </div>
}
