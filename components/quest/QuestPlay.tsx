...
import {advance,cast,emptyRound,eligibleFish,fight,hook,jstMonth,lure,setDrag,canLand,land} from '@/lib/quest/engine';
...
export default function QuestPlay({initialFish='',initialMethod=''}:{initialFish?:string;initialMethod?:string}){
 ...
 const busy=['waiting','bite','fight','landing'].includes(round.phase);
 ...
 const message={ready:'釣り場と仕掛けを選んで、キャスト。',waiting:round.style==='sabiki'?'群れの棚を探そう。':round.style==='eging'?'しゃくりとフォールで誘おう。':round.style==='kawahagi'?'誘って、つつく動きを観察しよう。':'魚が近づく様子を見よう。',bite:'アタリ！今、合わせよう。',fight:'掛かった！水面へ寄せよう。',landing:'LANDING — 魚を取り込む！',caught:'釣り上げた！',miss:round.reason}[round.phase];
 const step={ready:1,waiting:2,bite:3,fight:5,landing:6,caught:6,miss:2}[round.phase];
 ...
 return <><h1 className={s.heading}>釣りに行く</h1><p>今月は{now?jstMonth(now):'—'}月。図鑑の釣れる時期に合う魚は、ゲームでも出会いやすくなります。</p><ol className={s.steps} aria-label="釣りの進行">{['釣り場','仕掛け','キャスト','アタリ','合わせ','ファイト','釣果'].map((label,i)=><li key={label} aria-current={i===step?'step':undefined}>{i+1} {label}</li>)}</ol>
  ...
  {round.phase==='waiting'&&<p>{round.style==='sabiki'?'群れと同じ棚へ仕掛けを上下させよう。魚が餌を食べてラインを引いたら合わせる。':round.style==='eging'?'「しゃくる」で2回ほど誘い、ボタンを離してフォールを待つ。イカがエギを抱いたところで合わせよう。':round.style==='kawahagi'?'小さく2回誘って止めよう。つつく前アタリは待ち、餌を持ち込む短い瞬間を掛ける。':round.style==='ayu'?'縄張りのアユがおとりへ近づき、追い払う瞬間を見よう。':'魚が餌へ近づく様子と、ラインの変化を見よう。'}</p>}
  {round.phase==='bite'&&<p>合わせの猶予：{Math.max(0,(round.deadline-now)/1000).toFixed(1)}秒</p>}
  {round.phase==='fight'&&<p className={s.note}>深度 {round.fishDepth<.8?'水面':round.fishDepth.toFixed(1)+'m'} · 距離 {round.distance.toFixed(1)}m{round.finalRunDone&&now<round.finalRunUntil?' · 最後の抵抗！':''}</p>}
  {round.phase==='fight'&&<div className={s.meters}><label>魚のスタミナ {Math.round(round.stamina)} / {round.fish?.stamina??75}<progress aria-label="魚のスタミナ" value={round.stamina} max={round.fish?.stamina??75}/></label><label>水面へ寄せる {Math.round(round.progress)}%<progress aria-label="取り込み" value={round.progress} max={100}/></label><label>テンション {Math.round(round.tension)} / 100<meter aria-label="テンション" min={0} max={100} low={10} high={70} optimum={40} value={round.tension}/></label></div>}
  ...
  {round.phase==='fight'&&<div className={s.actions}><button className={s.primary} disabled={!canLand(round)} onClick={()=>setRound(r=>land(r,Date.now()))}>{round.landingMode==='net'?'タモで取り込む':'抜き上げる'}{!canLand(round)?' — 水面まで寄せて糸を整える':''}</button></div>}
