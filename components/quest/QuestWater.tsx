...
export default function QuestWater({round,now}:{round:Round;now:number}){
 ...
 const x=finishing?40:fight?Math.max(38,(round.running?68:48)-round.progress*.22):bite?(threeD?32:40):72-round.attraction*(threeD?.4:.32);
 ...
 return <div ref={waterRef} className={s.water} data-phase={round.phase} aria-label="水中の様子">
  ...
  <svg className={s.line} viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true"><path d={`M 10 4 Q 27 ${bite||fight?28:4} 43 20 L ${fight||finishing?mouthX:43} ${fight||finishing?y:baitY}`} fill="none" stroke={bite?'#ffd673':'#e0f9ff'} strokeWidth={bite?'.7':'.3'}/><path d={`M 10 4 Q 27 ${bite||fight?28:4} 43 20`} fill="none" stroke="#bd8560" strokeWidth="1.3"/></svg>
  ...
  {engaged&&<><span className={`${s.bait} ${bite?s.bitten:''}`} style={{left: `${fight||finishing?mouthX:43}%`,top:`${fight||finishing?y:baitY}%`}} aria-label={round.style==='eging'?'エギ':round.style==='ayu'?'おとり':'餌'}>{round.style==='eging'?'◁':round.style==='ayu'?'◁':'●'}</span><div className={`${s.fish} ${fight&&round.running?s.running:''}`} style={{left:`${x-fishWidth/2}%`,top:`${y}%`,width:`${fishWidth}%`,height:finishing?150:fight?100+near*45:undefined,animationDuration:`${2/(f?.swimSpeed??1)}s`}}>{threeD&&f?.modelSrc?<Viewer modelSrc={f.modelSrc} modelTilt={f.modelTilt} contain swim/>:f?.image?<ProfileImage optimized src={f.image} alt={`${f.name}が${pose}`} sizes="(max-width:760px) 180px, 360px"/>:<FishArt slug={f?.slug??''} label={f?.name??'魚影'}/>}</div></>}
