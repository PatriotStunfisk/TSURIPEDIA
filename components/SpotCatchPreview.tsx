...
export default function SpotCatchPreview({slug,fishNames,canPost=true,onDetails,compact=false}:{compact?:boolean;onDetails?:()=>void;slug:string;fishNames:Record<string,string>;canPost?:boolean}){
 ...
 return <div className={s.catchPreview}>{(!compact||state.reports.length>0)&&<b>UOLINKの釣果</b>}{compact&&!state.reports.length?null:state.loading?<span>確認中…</span>:state.error?<span>釣果は詳細ページで確認できます</span>:state.reports.length?state.reports.slice(0,compact?1:2).map(r=><a key={r.id} href={`/spots/${slug}#catches`} onClick={onDetails?event=>{event.preventDefault();onDetails();}:undefined}>{fishNames[r.fishSlug]||r.fishName||(r.fishSlug==='other'?'その他':'魚種不明')} {r.sizeLabel||(r.sizeCm?`${r.sizeCm}cm`:'')} ×{r.count} <time>{r.date.slice(5).replace('-','/')}</time></a>):<span>最初の釣果を投稿しませんか</span>}<div><a href={`/spots/${slug}#catches`} onClick={onDetails?event=>{event.preventDefault();onDetails();}:undefined}>釣果をもっと見る</a>{canPost&&<a href={`/spots/${slug}#catch-form`}>＋ 釣果を投稿</a>}</div></div>;
}
