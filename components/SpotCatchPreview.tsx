...
export default function SpotCatchPreview({slug,fishNames,canPost=true}:{slug:string;fishNames:Record<string,string>;canPost?:boolean}){
 ...
 return <div className={s.catchPreview}><b>最近の釣果</b>{state.loading?<span>確認中…</span>:state.error?<span>釣果は詳細ページで確認できます</span>:state.reports.length?state.reports.map(r=><a key={r.id} href={`/spots/${slug}#catches`}>{fishNames[r.fishSlug]||r.fishName||(r.fishSlug==='other'?'その他':'魚種不明')} {r.sizeLabel||(r.sizeCm?`${r.sizeCm}cm`:'')} ×{r.count} <time>{r.date.slice(5).replace('-','/')}</time></a>):<span>最初の釣果を投稿しませんか</span>}<div><a href={`/spots/${slug}#catches`}>釣果をもっと見る</a>{canPost&&<a href={`/spots/${slug}#catch-form`}>＋ 釣果を投稿</a>}</div></div>;
}
