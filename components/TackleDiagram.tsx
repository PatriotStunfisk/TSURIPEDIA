...
export default function TackleDiagram({slug}:{slug:string}){
 ...
 return <section className={s.wrap} aria-label={`${method.name}の仕掛け図解`}>
  ...
  <div className={s.layout}><figure className={s.diagram}>
   ...
   <ol><li><RigIcon kind="line"/><div><b>メインライン</b><span>{method.line}</span></div></li>
    {!rig.leaderInRig&&slug!=='fukase'&&<li><RigIcon kind="line"/><div><b>リーダー / ハリス</b><span>{method.leader}</span></div></li>}
  ...
  </figure><aside className={s.starter}><small>FIRST SET</small><h3>最初の1セット</h3><p>まず揃える道具と、選ぶ範囲の目安です。</p><dl>{specs.map(v=><div key={v.key}><dt>{v.label}</dt><dd>{v.value}</dd></div>)}</dl><p className={s.note}>{rig.caution}</p><p>サイズは地域・魚・潮・水深で変わります。釣り場と船宿の指定を優先してください。</p><Link href={`/gear?method=${slug}`}>このタックルを揃える →</Link><Link href="/guide/tackle-connection-basics">接続と結び目の確認方法 →</Link></aside></div>
 </section>;
}
