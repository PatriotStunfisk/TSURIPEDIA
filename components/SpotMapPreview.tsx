...
export default function SpotMapPreview({entry,shop,onClose,onDetails}:{entry?:FishingMapEntry;shop?:TackleShop;onClose:()=>void;onDetails:()=>void}){
 ...
 return <section className={s.mapPreview} aria-label="選択地点のプレビュー" onKeyDown={event=>{if(event.key==='Escape'){event.stopPropagation();onClose();}}}>
  ...
  {entry&&<div className={s.previewFish}>{entry.fish.slice(0,4).map(fish=><span key={fish}>{fish}</span>)}</div>}
