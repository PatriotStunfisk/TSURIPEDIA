...
export default function MadaiViewer(){
 ...
 useEffect(()=>{
  ...
  const pivot=new THREE.Group();scene.add(pivot);const normalized=new THREE.Group();pivot.add(normalized);
  let targetY=0,targetX=0,ry=0,rx=0,down=false,lastX=0,lastY=0;
  const centerAndFit=(obj:THREE.Object3D)=>{normalized.add(obj);obj.updateMatrixWorld(true);const box=new THREE.Box3().setFromObject(obj);const size=box.getSize(new THREE.Vector3());const center=box.getCenter(new THREE.Vector3());obj.position.x-=center.x;obj.position.y-=center.y;obj.position.z-=center.z;const max=Math.max(size.x,size.y,size.z)||1;normalized.scale.setScalar(3.75/max);normalized.updateMatrixWorld(true);camera.position.set(0,0,5.35);camera.lookAt(0,0,0)};
 ...
 return <div ref={ref} style={{position:'relative',width:'100%',height:'100%',minHeight:260,overflow:'hidden',background:'transparent'}}>{status==='loading'&&<div style={{position:'absolute',inset:0,display:'grid',placeItems:'center',zIndex:2,color:'#d8f5ff'}}>3Dモデルを読み込み中…</div>}{status==='failed'&&<div style={{position:'absolute',inset:0,display:'grid',placeItems:'center',alignContent:'center',gap:8,zIndex:2,color:'#d8f5ff',textAlign:'center',padding:24}}><b>3Dモデルを読み込めませんでした</b>{detail&&<small style={{opacity:.72,fontSize:10,wordBreak:'break-word'}}>{detail}</small>}</div>}</div>
}
