...
export function ActivityReason({activity,fishNames,compact=false}:{activity:SpotActivity;fishNames:Record<string,string>;compact?:boolean}){return <p style={{fontSize:12,margin:'8px 0'}}><b>{activity.level==='hot'?'🔥 最近の釣果が集まっています':'最近の釣果あり'}</b><br/>直近7日：{activity.count7}件 · 30日：{activity.count30}件{!compact&&<><br/>{activity.fish.map(f=>`${fishNames[f.slug]??'その他'} ${f.count}`).join(' / ')}<small>（30日・報告件数）</small></>}</p>;}
