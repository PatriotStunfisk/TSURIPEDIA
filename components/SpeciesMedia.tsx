...
export default function SpeciesMedia({name,image,modelSrc,modelTilt}:{name:string;image:string;modelSrc?:string;modelTilt?:number}){
 ...
 return <div style={{position:'absolute',inset:0,overflow:'hidden'}}>
  ...
  <div style={{position:'absolute',inset:0,padding:'54px 22px 48px',display:'flex',alignItems:'center',justifyContent:'center'}}>
   {mode==='3d'&&modelSrc?<Viewer modelSrc={modelSrc} modelTilt={modelTilt}/>:<ProfileImage optimized loading="eager" fetchPriority="high" sizes="(max-width: 800px) 100vw, 70vw" src={image} alt={`${name}の図鑑画像`} width={1536} height={1024} style={{width:'100%',height:'100%',objectFit:'contain'}}/>}
  </div>
 </div>
}
