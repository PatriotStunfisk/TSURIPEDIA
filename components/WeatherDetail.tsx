...
export default function WeatherDetail(){
 ...
 <WeatherPlaceSearch onSelect={p=>{setSpot(p);window.history.replaceState(null,'',`?spot=${encodeURIComponent(p.id)}`)}}/><WeatherJapanMap selected={spot?'':city} onSelect={id=>{setCity(id);setSpot(null);window.history.replaceState(null,'',`?city=${id}`)}}/>{spot?<section className={s.selectedPlace}><h2>{spot.name}の天気</h2><p>{spot.area}</p><a href={`/spots/${spot.id}`}>釣り場情報を見る →</a><WeatherHourly key={spot.id} city={city} date={date} spot={spot.id}/><p className={s.hourlyNote}>登録地点の座標に基づく予報です。この地点の潮汐・波高は未対応です。海域の予報は地図上の主要都市から確認できます。</p></section>:<><div className={s.body} aria-live="polite"><div><small>{date} · {place.name}</small>{!data&&!failed?<p>天気を読み込み中…</p>:<><p className={s.weather}>{data?.forecast&&<WeatherIcon text={data.forecast.weather}/>} {data?.forecast?.weather??'天気データを取得できませんでした'}</p>{data?.forecast&&<p>降水確率 {data.forecast.rain!==undefined?`${data.forecast.rain}%（日内最大）`:'—'}</p>}{!compact&&data?.forecast&&<><p>風：{data.forecast.wind??'情報なし'}</p><p>波：{data.forecast.wave??'情報なし'}</p><small>発表：{new Date(data.forecast.published).toLocaleString('ja-JP',{timeZone:'Asia/Tokyo'})}</small></>}</>}</div><div><small>潮汐地点：{place.port} · 予測潮位</small>{data?.tide?<><TideGraph tide={data.tide} port={place.port}/>{!compact&&<div className={s.events}><p>満潮 {data.tide.high.map(e=>`${e.time}（${e.height}cm）`).join(' / ')||'この日は予測なし'}</p><p>干潮 {data.tide.low.map(e=>`${e.time}（${e.height}cm）`).join(' / ')||'この日は予測なし'}</p></div>}</>:<p>{!data&&!failed?'潮汐を読み込み中…':'潮汐データを取得できませんでした'}</p>}</div></div>
