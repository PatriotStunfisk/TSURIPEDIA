...
export default function SpotMap({mapEntries,shops=[],nearSpot,initialShops=false,initialFavorites=false,initialQuery='',initialKind='all',fishNames={},methodNames={},guideNames={},cookingSlugs=[]}:{mapEntries:FishingMapEntry[];shops?:TackleShop[];nearSpot?:string;initialShops?:boolean;initialFavorites?:boolean;initialKind?:'all'|MapEntryType;initialQuery?:string;fishNames?:Record<string,string>;methodNames?:Record<string,string>;guideNames?:Record<string,string>;cookingSlugs?:string[]}){
 ...
 const selectSpot=(slug:string)=>{setSelectedShop('');setSelected(slug);requestAnimationFrame(()=>detailRef.current?.scrollIntoView({block:'start',behavior:'smooth'}));};
 ...
 const visibleShops=useMemo(()=>!shopsOn?[]:anchor?nearby.map(x=>x.shop):shops.filter(x=>x.status==='open'&&(!prefecture||x.prefecture===prefecture)&&(!region||(japanRegions[region as JapanRegion] as readonly string[]).includes(x.prefecture))),[shopsOn,anchor,nearby,shops,prefecture,region]);
 const activeShop=visibleShops.find(x=>x.id===selectedShop);
 const filteredEntries=useMemo(()=>mapEntries.filter(e=>(!favoritesOnly||favorites.includes(e.slug))&&(!anchor||!hasCoordinates(anchor)||(hasCoordinates(e)&&distanceKm(anchor,e)<=30))&&selectedTypes.includes(markerKind(e))&&matchesSpot(e,{region,prefecture,method,terrain,beginner,family,showClosed,kind,fish,query})),[mapEntries,favoritesOnly,favorites,anchor,selectedTypes,region,prefecture,method,terrain,beginner,family,showClosed,kind,fish,query]);
 ...
 return <section className={s.wrap}>
  <div className={s.nationalFilters}>
   ...
   <button onClick={()=>{setFavoritesOnly(false);setRegion('');setPrefecture('');setMethod('');setTerrain('');setBeginner(false);setFamily(false);setFish('すべて');setQuery('');setKind('all');setShowClosed(false);setShopsOn(false);setSelectedShop('');setSelectedTypes(Object.keys(markerKinds) as SpotPrimaryType[]);}}>絞り込みをリセット</button>
  </div><p className={s.resultCount} role="status">登録情報から {entries.length} 件。未登録の地域は順次追加しています。</p>
  <div className={s.favoriteBar}><button aria-pressed={favoritesOnly} onClick={()=>{setFavoritesOnly(!favoritesOnly);setListLimit(20);}}>★ お気に入り {favorites.length}</button><small>この端末に保存</small></div>
  ...
  <div className={s.contentGrid}>
   ...
   {shopsOn&&<section><h2>登録されている釣具店</h2><div className={s.shopList}>{visibleShops.map(shop=><button className={s.spotCard} key={shop.id} onClick={()=>{setSelectedShop(shop.id);requestAnimationFrame(()=>detailRef.current?.scrollIntoView({block:'start',behavior:'smooth'}));}}><strong>店 {shop.name}</strong><p>{shop.address}</p>{anchor&&hasCoordinates(anchor)&&<small>約{distanceKm(anchor,shop).toFixed(1)}km（直線）</small>}</button>)}</div>{!visibleShops.length&&<p>この範囲には確認済みの登録店舗がまだありません。店舗が存在しないという意味ではありません。</p>}</section>}<div className={s.spotList}>{entries.length?entries.slice(0,listLimit).map(e=><div key={e.slug} className={s.favoriteCard}><button aria-pressed={active?.slug===e.slug} className={`${s.spotCard} ${active?.slug===e.slug?s.selected:''}`} onClick={()=>selectSpot(e.slug)}>
   ...
   </button><SpotFavorite slug={e.slug} name={e.name}/></div>):<div className={s.empty}>{favoritesOnly?'お気に入りに登録した釣り場がないか、現在の絞り込み条件に一致しません。':'条件に合う釣り場はありません。'}休業・閉鎖情報を探す場合は「休業・閉鎖情報も表示」を選んでください。</div>}</div>
