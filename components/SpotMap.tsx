...
export default function SpotMap({mapEntries,shops=[],nearSpot,initialShops=false,initialQuery='',initialKind='all',fishNames={},methodNames={},guideNames={},cookingSlugs=[]}:{mapEntries:FishingMapEntry[];shops?:TackleShop[];nearSpot?:string;initialShops?:boolean;initialKind?:'all'|MapEntryType;initialQuery?:string;fishNames?:Record<string,string>;methodNames?:Record<string,string>;guideNames?:Record<string,string>;cookingSlugs?:string[]}){
 ...
 const filteredEntries=useMemo(()=>mapEntries.filter(e=>(!anchor||!hasCoordinates(anchor)||(hasCoordinates(e)&&distanceKm(anchor,e)<=30))&&selectedTypes.includes(markerKind(e))&&matchesSpot(e,{region,prefecture,method,terrain,beginner,family,showClosed,kind,fish,query})),[mapEntries,anchor,selectedTypes,region,prefecture,method,terrain,beginner,family,showClosed,kind,fish,query]);
 ...
 return <section className={s.wrap}>
  <div className={s.nationalFilters}>
   ...
   <button onClick={()=>{setRegion('');setPrefecture('');setMethod('');setTerrain('');setBeginner(false);setFamily(false);setFish('すべて');setQuery('');setKind('all');setShowClosed(false);setShopsOn(false);setSelectedShop('');setSelectedTypes(Object.keys(markerKinds) as SpotPrimaryType[]);}}>絞り込みをリセット</button>
  ...
  <InteractiveSpotMap shops={visibleShops} selectedShop={activeShop?.id} onShopSelect={id=>{setSelectedShop(id);requestAnimationFrame(()=>detailRef.current?.scrollIntoView({block:'start',behavior:'smooth'}));}} focus={anchor&&hasCoordinates(anchor)?anchor:undefined} fitKey={[nearSpot,region,prefecture,query,fish,method,terrain,kind].join("|")} selectedTypes={selectedTypes} onTypesChange={setSelectedTypes} entries={entries} selected={active?.slug} onSelect={selectSpot}/>
  <label className={s.shopSwitch}><input type="checkbox" checked={shopsOn} onChange={e=>setShopsOn(e.target.checked)}/>店 釣具店（{visibleShops.length}件表示）</label>{shopsOn&&<p>オレンジの四角は釣具店です。営業時間・餌の在庫は公式案内で確認してください。登録店舗がない地域もあります。</p>}
