...
export default function AjiViewer(){
 ...
 useEffect(()=>{
  ...
  const centerAndFit=(obj:THREE.Object3D)=>{
   normalized.add(obj);obj.updateMatrixWorld(true);
   ...
   const max=Math.max(size.x,size.y,size.z)||1;normalized.scale.setScalar(3.75/max);normalized.updateMatrixWorld(true);
  ...
  loader.load('/models/aji.glb?v=20260912-1',g=>{
   if(disposed)return;g.scene.traverse(o=>{if((o as THREE.Mesh).isMesh)(o as THREE.Mesh).frustumCulled=false});centerAndFit(g.scene);setStatus('ready');setDetail('');
