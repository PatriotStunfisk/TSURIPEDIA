...
export default function RecipeDeepDive({detail}:{detail:RecipeDetail}) {
 return <section id="recipe-details" className={styles.section}>
  ...
  {detail.sources.length>0&&<details className={styles.sources}><summary>参考資料</summary><ul>{detail.sources.map(source=><li key={source.url}><a href={source.url} target="_blank" rel="noopener noreferrer">{source.label} ↗</a></li>)}</ul></details>}
 </section>;
}
