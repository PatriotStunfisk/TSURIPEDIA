...
export function pageSharing(path:string,title:string,description:string,type:'website'|'article'='website'):Metadata{
 ...
 return {title,description,alternates:{canonical:new URL(path,siteUrl).href,languages:languageAlternates(path)},openGraph:{title,description,url:new URL(path,siteUrl).href,type,siteName:'UOLINK（ウオリンク）',locale:'ja_JP',images:[image]},twitter:{card:'summary_large_image',site:'@uo_link',title,description,images:[{url:image.url,alt:title}]}};
}
