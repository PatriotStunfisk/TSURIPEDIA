...
export function fishMatchesSearch(fish:{name:string;en:string;scientific:string;slug:string;detail?:{aliases:readonly string[]}},query:string){
 const words=[fish.name,fish.en,fish.scientific,...(fish.detail?.aliases??[]),...(additionalFishNames[fish.slug]??[])].map(normalizeFishSearch);
