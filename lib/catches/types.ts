...
export type CatchReport={id:string;source:'real';spotSlug:string;fishSlug:string;count:number;sizeCm?:number;date:string;time:keyof typeof catchTimes;methodSlug?:string;rig:string;comment:string;photo?:string;createdAt:string};
...
export function parseCatch(value:unknown,allowed:{fish:Set<string>;spots:Set<string>;methods:Set<string>},now=new Date()):CatchReport{
 ...
 if(!allowed.spots.has(spotSlug)||!allowed.fish.has(fishSlug)||(methodSlug&&!allowed.methods.has(methodSlug)))throw Error('釣り場・魚・釣法を選び直してください。');
 ...
 return {id,source:'real',spotSlug,fishSlug,count:v.count,sizeCm:v.sizeCm as number|undefined,date,time:time as keyof typeof catchTimes,methodSlug:methodSlug||undefined,rig:text('rig',100),comment:text('comment',500),photo:photo||undefined,createdAt:now.toISOString()};
}
