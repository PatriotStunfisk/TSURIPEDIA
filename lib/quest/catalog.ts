...
export const questFish:QuestFish[]=fishCatalog.flatMap(f=>{
 ...
 return [{slug:f.slug,name:f.name,months:f.months,season:f.season,sizeLabel:f.size.includes('kg')?'重量':f.size.includes('胴長')?'胴長':'全長',sizeUnit:f.size.includes('kg')?'kg':'cm',minSize,maxSize,
  ...
  hook:q?.hook,image:getFishImage(f),modelSrc:getSpeciesModelSrc(f.slug),modelTilt:getFishSpecies(f.slug)?.media?.modelTilt,cooking:!!f.cooking,guideHref:getFishConnections(f.slug)?.guides[0]?`/guide/${getFishConnections(f.slug)!.guides[0].slug}`:undefined}];
});
