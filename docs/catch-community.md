...
新規投稿はログイン・入力検証・回数制限・写真保存が完了した時点で公開します。事前承認は不要です。通報と管理者の非公開化、本人削除は維持します。以下の履歴にある承認待ちの手順は旧仕様です。
...
## 公開フィードとログイン不要のいいね
- `/catches` は公開投稿10件ずつの新着一覧。魚・都道府県・spot ID・釣法・釣行日でサーバー側絞り込み。自分のクラウド／端末記録は同じURL内の展開欄に保持。
- `supabase/catch-likes.sql` を適用。いいねはHttpOnly Cookieの匿名ブラウザIDをサーバーでHMAC化して保存し、投稿×ハッシュの一意制約で重複を防止。Cookie削除や別端末を同一人物と判定する機能ではない。
- Cookieはいいねを押した時のみ発行。生のIPやメールはいいねテーブルに保存しない。RLSと権限でanon/authenticatedからの直接アクセスを禁止し、公開済み・写真保存完了の投稿のみAPI経由で反応できる。CSRF確認とIP単位の回数制限を維持。
- 重複いいね・解除・再読み込みをローカルE2Eで検証。本番は公開投稿が0件のため反応操作を行わず、RLSと権限を確認。架空の釣果・いいねを本番に残さない。
