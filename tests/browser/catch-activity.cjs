...
(async()=>{
 ...
 for(const width of [1280,390]){
  ...
  await context.route('**/api/catches/external?*',r=>r.fulfill({json:{reports:['official',...Array(7).fill('external')].map((sourceType,index)=>({id:sourceType+index,source:'LOCAL FIXTURE',sourceType,date:new Date().toISOString().slice(0,10),fishSlug:'aji',spotSlug:'naruohama',summary:'LOCAL TEST ONLY',sourceUrl:'https://example.com/catch/'+sourceType,methodSlug:'sabiki',sizeCm:20,...(index===1?{count:7,countScope:'boat'}:{})}))}}));
  ...
  await page.goto('http://localhost:3118/catches',{waitUntil:'domcontentloaded'});await page.getByRole('heading',{name:'HOTな釣り場'}).waitFor();await page.getByText('🔥 最近の釣果が集まっています',{exact:true}).waitFor();await page.getByText('LOCAL FIXTURE',{exact:true}).waitFor();await page.locator('article').getByText('外部情報',{exact:true}).first().waitFor();await page.getByText('船中合計',{exact:true}).waitFor();assert.equal(await page.getByRole('link',{name:'出典の釣果を読む ↗'}).count(),1);assert.equal(await page.getByText('LOCAL TEST ONLY',{exact:true}).count(),0);await page.getByRole('button',{name:'残りの釣果を見る（2件）'}).click();await page.getByRole('button',{name:'表示を少なくする'}).click();await page.getByRole('button',{name:'外部情報',exact:true}).click();assert.equal(await page.getByRole('link',{name:'出典の釣果を読む ↗'}).count(),0);await page.getByRole('button',{name:'公式情報',exact:true}).click();assert.equal(await page.getByRole('link',{name:'出典の釣果を読む ↗'}).count(),1);await page.getByRole('button',{name:'すべて',exact:true}).click();await page.getByLabel('サイズ下限（cm）').fill('20');await page.getByLabel('サイズ上限（cm）').fill('80');assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1));
  await page.getByRole('heading',{name:'公式・外部の釣果',exact:true}).scrollIntoViewIfNeeded();await page.screenshot({path:`/tmp/uolink-catch-results-${width}.png`});
